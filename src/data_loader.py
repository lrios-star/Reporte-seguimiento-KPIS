...
# Regex para detectar hojas tipo "sem 32", "Sem 33", "Semana 34"
_PERIOD_PATTERN = re.compile(r"^\s*sem(?:ana)?\s*(\d{1,2})\s*$", re.IGNORECASE)


def _parse_period_name(name: str) -> Optional[int]:
    """Convierte un nombre de hoja tipo 'sem 32' en un número entero (32)."""
    match = _PERIOD_PATTERN.match(name)
    if not match:
        return None
    
    # Extrae el número y lo convierte a entero
...
def discover_periods(excel_path=None) -> list[dict]:
    """Descubre las hojas que representan periodos semanales (ej. sem 32, sem 33)."""
    ...
    for name in sheet_names:
        week_num = _parse_period_name(name)
        if week_num is not None:
            periods.append({
                "sheet_name": name,
                "period_date": week_num,  # <-- Mantenemos 'period_date' para que calculations.py no tire error
                "week_num": week_num,
                "label": name.strip(),
            })

    # Ordenar por el número de semana
    periods.sort(key=lambda p: p["week_num"])
    return periods
