...
def inject_custom_css():
    ...
    <style>
        ...
        /* ---- Logo en el header (sidebar colapsada) ---- */
        #logo-main-area {
        ...
        /* Método primario: CSS :has() detecta sidebar colapsada y muestra el logo */
        body:has(section[data-testid="stSidebar"][aria-expanded="false"]) #logo-main-area {
            display: block !important;
        }
    </style>
    """, unsafe_allow_html=True)

    # JS vía components.html — MutationObserver que complementa el CSS :has()
    # El logo ya vive dentro del header (render_header), no hay div separado aquí.
    components.html(
        """
        <script>
        (function() {
            ...
            function applyLogoState() {
                var logoDiv = window.parent.document.getElementById('logo-main-area');
                if (!logoDiv) return;
                logoDiv.style.display = isSidebarCollapsed() ? 'block' : 'none';
            }
            function startObserver() {
                var sb = getSidebar();
                if (!sb) { setTimeout(startObserver, 300); return; }
                applyLogoState();
                new MutationObserver(function() { applyLogoState(); })
...
def render_header(current_label: str, previous_label: str | None, logo_b64: str = ""):
    ...
    # Logo: siempre presente en el HTML pero oculto por CSS;
    # se muestra vía CSS :has() + JS cuando el sidebar está colapsado.
    if logo_b64:
        logo_section = (
            f'<div id="logo-main-area">'
            f'<img src="data:image/png;base64,{logo_b64}" alt="Logo">'
            f'<div class="logo-credits">Elaborado por: Luciano Ríos - Ing. De Procesos</div>'
            f'</div>'
        )
    else:
        logo_section = ""
